...
def to_request_data(field, data):
    ...
    for f in field.fields:
        try:
            request_data[f.nodename] = f.widget.to_request_data(f, data.get(f.name))
        except Invalid, e:
            f.errors[f.name] = e
            raise
    return request_data
...
def pre_parse_incoming_request_data(field, request_data):
    """
    ...
    for f in field.fields:
        if should_recurse(f):
            data[f.nodename] = pre_parse_incoming_request_data(f, request_data)
        else: 
            # This needs to be cleverer...
            item_data = request_data.get(f.name, None)
            data[f.nodename] = f.widget.pre_parse_incoming_request_data(f, item_data, full_request_data=request_data)
